<!---<cfquery name="doUKsale" datasource="#DSN#">
    ...
    where ONHAND>0 AND albumStatusID<25 AND albumStatusID>21 AND NRECSINSET=1 AND countryID=3 AND blue99=0
</cfquery>//--->
<!---<cfquery name="priceAdjust" datasource="#DSN#">
	select ID, priceSave
    from catItems
    where NRECSINSET=1 AND countryID=3 AND blue99=0 AND priceSave<>0
</cfquery>
<table>
<cfoutput>
<cfloop query="priceAdjust">
	...
	<cfquery name="undoUKsale" datasource="#DSN#">
    	update catItems
        set priceSave=<cfqueryparam cfsqltype="cf_sql_float" value="#newPrice#">, specialItem=0
        where ID=<cfqueryparam cfsqltype="cf_sql_integer" value="#ID#">
    </cfquery>
     
</cfloop>
</cfoutput>//--->
<cfquery name="undoGMBHsale" datasource="#DSN#">
        update catItems
