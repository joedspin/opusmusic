...
<cfparam name="url.genreFilter" default="0">
<cfparam name="url.catalogFilter" default="">
<cfparam name="url.includeImports" default="false">
<cfparam name="url.showAll" default="false">
<cfparam name="url.current" default="no">
<style>
<!--
...
<cfif url.genreFilter NEQ "0"><cfset genreFilterString="AND genreID=#url.genreFilter#"><cfelse><cfset genreFilterString=""></cfif>
...
<cfif url.catalogFilter EQ "classics">
	    <cfquery name="distro" datasource="#DSN#">
        SELECT * from catItemsQuery
        where albumStatusID<25 AND ONHAND>0 AND mediaID NOT IN (6,19,20,21,22,23,24) AND (vendorID IN (5650,6978,5726,5439,7848,5697,2811) OR shelfID IN (1059,1066,2075,34,40,2136,2127) OR reissue=1) AND (shelfID NOT IN (15,2108))
        order by artist, catnum
    </cfquery>
<cfelse>
    <cfquery name="distro" datasource="#DSN#">
        SELECT * from catItemsQuery
        where albumStatusID<25 AND ONHAND>0 AND mediaID NOT IN (19,21,22,23,24) AND shelfID NOT IN (15) #shelfFilterString# #vendorFilterString# #genreFilterString# #importFilterString#
