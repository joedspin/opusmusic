...
<cfquery name="thisItems" datasource="#DSN#">
	SELECT catnum, Sum(qtyOrdered) As totalOrdered, label, artist, title, ONSIDE, ONHAND, dtID
	FROM orderItemsQuery
	where adminAvailID=2 AND statusID<6AND statusID>1 AND Left(shelfCode,1)='D'
	group by catnum, label, artist, title, ONSIDE, ONHAND, dtID
...
<form name="picklist" method="ordersPicklistDT161Load.cfm" method="post">
