...
<cfif Session.orderID NEQ "0">
  ...
  <cfif form.promoID EQ 0>
	<cfset dopromo=false>
		...
		<cfif checkPromo EQ "CHART304" OR checkPromo EQ "MJ5" OR checkPromo EQ "DT3045" OR (checkPromo EQ "JOETEN" AND DateFormat(varDateODBC,"yyyy-mm-dd") LT "2009-11-05")OR (checkPromo EQ "TEN304" AND DateFormat(varDateODBC,"yyyy-mm-dd") LT "2010-01-01") OR (checkPromo EQ "DEEP510" AND DateFormat(varDateODBC,"yyyy-mm-dd") LTE "2009-10-31") OR (checkPromo EQ "NICE5" AND DateFormat(varDateODBC,"yyyy-mm-dd") LTE "2010-03-31") OR (checkPromo EQ "HOUSE161" AND DateFormat(varDateODBC,"yyyy-mm-dd") LTE "2009-12-31") OR (checkPromo EQ"DISCOGSDT" OR checkPromo EQ "AMAZONDT" OR checkPromo EQ "MUSICSTACK" ) ><!---  OR (checkPromo EQ "VOTE" AND DateFormat(varDateODBC,"yyyy-mm-dd") LT "2008-11-06") OR (DateFormat(varDateODBC,"yyyy-mm-dd") LT "2008-11-06")//--->
		<cfset dopromo=true>
		...
		<cfif checkPromo EQ "MJ5" OR checkPromo EQ "DT3045" OR checkPromo EQ "CHART304" OR checkPromo EQ "VINYL509">
             <cfloop query="cartContents">
                <cfset itemID=orderItemID>
                <cfif price GT 6.99>
...
<cfelseif form.promoID GT 0 AND IsNumeric(form.promoID)>
    ...
    AND adminAvailID=2 AND ((catItemID>=49118 AND catItemID<=49122) OR (catItemID IN (44545,44547,44581,44582,44584,44585,44586,44587)) OR (catItemID>=54297 AND catItemID<=54301))
  </cfquery>
...
<cfquery name="cartContents" datasource="#DSN#">
	...
	<cfif DateFormat(varDateODBC,"yyyy-mm-dd") LTE "2010-04-22">
		<cfif shipCost GT 0 and form.shipOrderSub GTE 50>
            ...
            <cfset thisPromo="Volcano Shipping Promotion">
            <cfset doPromo=true>
         </cfif>
        </cfif><!---//--->
