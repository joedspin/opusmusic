...
<cfquery name="Application.dt161Items" datasource="#DSN#" cachedwithin="#CreateTimeSpan(1, 0, 0, 0)#">
		select *
		from catItemsQuery
...
<cfif thisSearchField EQ "all">
	...
	<cfelse><!--find search string in all//-->
		<cfquery name="dt161daily"  datasource="#DSN#">
			select *
			from catItemsQuery
			where (artist LIKE <cfqueryparam value="%#form.searchString#%" cfsqltype="cf_sql_char"> OR
			label LIKE <cfqueryparam value="%#form.searchString#%" cfsqltype="cf_sql_char"> OR
			title LIKE <cfqueryparam value="%#form.searchString#%" cfsqltype="cf_sql_char">)
			AND Left(shelfCode,1)='D' AND shelfCode<>'DO'
...
<cfelseif thisSearchField NEQ "">
	...
	<cfquery name="dt161daily" datasource="#DSN#">
		select *
		from catItemsQuery
		where <cfqueryparam value="#thisSearchField#" cfsqltype="cf_sql_integer"> LIKE <cfqueryparam value="%#thisSearchString#%" cfsqltype="cf_sql_char">
	</cfquery>
	</cfif>
<cfelse>
	<cfif url.stat EQ 24><!--find 24//-->
		<cfquery name="dt161daily" dbtype="query">
			select *
			from Application.dt161Items
			where (albumStatusID=24) AND (mediaID<2 OR mediaID>3)
	...
	<cfelse><!--find 25//-->
		   <cfquery name="dt161daily" dbtype="query">
			select *
			from Application.dt161Items
			where (albumStatusID=#url.stat#) AND (mediaID<2 OR mediaID>3)
			 order by dtDateUpdated DESC, label ASC
		  </cfquery>
	</cfif>
</cfif>
<cfif dt161daily.recordCount GT 30>
<cfset numPages=Int((dt161daily.recordCount-1)/30)+1>
<cfoutput>
...
<cfoutput query="dt161daily" maxrows="30" startrow="#thisStartRow#">
  ...
  <tr>
    ...
    <td width="750" align="left" valign="top" class="detailsArtist" ><b>#artist#</b>
		<br />#title#</td>
