...
<cfparam name="form.ret" default="next">
...
<cfif form.cardChoice EQ 0 AND cardExpDate LT DateFormat(varDateODBC,"yyyy-mm")><cfinclude template="checkOutPageHead.cfm"><p>Please click "Back" and enter a new new expiration date for your card. The date you entered is already expired.</p><cfinclude template="checkOutPageFoot.cfm"><cfabort></cfif>
...
<cfif form.billID NEQ 0 AND form.shipID NEQ 0>
    ...
    <cfif form.ret EQ "pcards">
    	<cflocation url="http://www.downtown304.com/index.cfm?managecards=true">
	<cfelseif form.cardChoice EQ "P">
		<cflocation url="checkOutPayPal.cfm?orderID=#thisOrder.ID#&thisPromo=#URLEncodedFormat(form.thisPromo)#&dsu=#URLEncodedFormat(Encrypt(Session.userID,'y6DD3cxo86zHGO'))#">
	<cfelse>
